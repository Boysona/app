...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "7770743573:AAELOYGUgVT_AhA1SHPzLszK0IjwOVKz7p8")
...
def send_summary_or_file(summary, chat_id, reply_to_message_id=None, filename_prefix='summary'):
    if summary is None:
        return
    if len(summary) <= 4000:
        if reply_to_message_id:
            bot.send_message(chat_id, summary, reply_to_message_id=reply_to_message_id)
        else:
        ...
        bio.seek(0)
        if reply_to_message_id:
            bot.send_document(chat_id, document=bio, reply_to_message_id=reply_to_message_id)
        else:
            bot.send_document(chat_id, document=bio)

def send_transcript_and_summary(transcript, summary, chat_id, reply_to_message_id=None, transcript_prefix='transcript', summary_prefix='summary'):
    if transcript is not None and transcript != "":
        if len(transcript) <= 4000:
            try:
                if reply_to_message_id:
                    bot.send_message(chat_id, transcript, reply_to_message_id=reply_to_message_id)
                else:
...
def handle_media_common(message):
    ...
    if message.voice:
        ...
        if mime and ("audio" in mime or "video" in mime):
            ...
            bot.send_message(message.chat.id, "Sorry, I can only process audio or video files.")
            return
    ...
    if file_size and file_size > TELEGRAM_MAX_BYTES:
        ...
        text = (
            "⚠️ The file is too large! upload here "
            f" <a href=\"{upload_link}\">here</a>"
        )
        ...
        try:
            ...
            try:
                ...
                if "ffmpeg" in error_msg.lower():
                    bot.send_message(message.chat.id, "⚠️ Server error: ffmpeg not found or conversion failed. Contact admin @boyso.", reply_to_message_id=message.message_id)
            ...
            send_transcript_and_summary(text, summary, message.chat.id, reply_to_message_id=message.message_id)
...
<body>
    <div class="container">
        ...
        <div class="file-upload-wrapper">
            <input type="file" id="file-input" class="file-upload-input" accept="audio/*,video/*">
...
def upload_large_file(token):
    ...
    def bytes_to_tempfile(b):
        ...
        tmp.close()
        return tmp.name
    def process_uploaded_file(chat_id_inner, lang_inner, path):
        try:
            try:
            ...
            try:
                send_transcript_and_summary(text, summary, chat_id_inner)
                increment_processing_count(str(chat_id_inner), "stt")
            except Exception:
                try:
                    bot.send_message(chat_id_inner, "Error sending summary message. The process completed but could not be delivered as a message.")
                except Exception:
                    pass
                return
        finally:
...
def handle_lang(message):
    try:
        ...
        bot.send_message(message.chat.id, "Choose a language for your Audio & Video:", reply_markup=kb)
    except Exception:
...
def handle_new_chat_members(message):
    try:
        if message.new_chat_members[0].id == bot.get_me().id:
            ...
            bot.send_message(message.chat.id, "Thanks for adding me! I'm ready to summarize your media files.")
    except Exception:
...
def handle_media_types(message):
    try:
        if message.chat.id == ADMIN_ID and admin_broadcast_state.get(message.chat.id, False):
            bot.send_message(message.chat.id, "Broadcasting your media now...")
...
def admin_inline_callback(call):
    try:
        ...
        if call.data == "admin_total_users":
            ...
            bot.edit_message_text(f"Total users registered: {total_users}", chat_id=call.message.chat.id, message_id=call.message.message_id)
            bot.send_message(
            ...
            bot.send_message(call.message.chat.id, "Okay, Admin. Send me the message (text, photo, video, document, etc.) you want to broadcast to all users. To cancel, type /cancel_broadcast")
...
@bot.message_handler(commands=['cancel_broadcast'], func=lambda message: message.chat.id == ADMIN_ID and admin_broadcast_state.get(message.chat.id, False))
...
@bot.message_handler(content_types=['text', 'photo', 'video', 'document', 'audio', 'voice'], func=lambda message: message.chat.id == ADMIN_ID and admin_broadcast_state.get(message.chat.id, False))
...
def handle_text_messages(message):
    try:
        if message.chat.id == ADMIN_ID and not admin_broadcast_state.get(message.chat.id, False):
            ...
            return
        bot.send_message(message.chat.id, "I can only process audio, video, or document files to provide a summary. Please send one of those, or use /lang to change your language settings.")
    except Exception:
...
def set_webhook_on_startup():
    try:
        ...
        time.sleep(0.2)
        bot.set_webhook(url=url)
        logging.info(f"Bot webhook set successfully to {url}")
    except Exception as e:
        logging.error(f"Failed to set bot webhook on startup: {e}")
