...
def update_user_activity(user_id: int):
    ...
    now = datetime.now()
    u = users_store.get(user_id_str)
    if u:
        u["last_active"] = now
    else:
        users_store[user_id_str] = {"first_seen": now, "last_active": now, "stt_conversion_count": 0, "stt_language": "en"}
...
def increment_processing_count(user_id: str, service_type: str):
    ...
    if not u:
        users_store[uid] = {"first_seen": now, "last_active": now, "stt_conversion_count": 0, "stt_language": "en"}
        u = users_store[uid]
...
def set_stt_user_lang(user_id: str, lang_code: str):
    ...
    u = users_store.get(uid)
    if not u:
        users_store[uid] = {"first_seen": now, "last_active": now, "stt_conversion_count": 0, "stt_language": lang_code}
    else:
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    if file_size and file_size > TELEGRAM_MAX_BYTES:
        ...
        text = (
            "⚠️ File too large! maximum allowed size is 20MB.\n👉 Use MediaToTextBot for files up to 2GB."
        )
    ...
    def handle_new_chat_members(message):
        try:
            if message.new_chat_members[0].id == bot_obj.get_me().id:
                ...
                groups_store[group_data['_id']] = group_data
    ...
    def handle_media_types(message):
        try:
            if message.chat.id == ADMIN_ID and admin_broadcast_state.get(message.chat.id, False):
                ...
                all_users_chat_ids = list(users_store.keys())
                sent_count = 0
                failed_count = 0
