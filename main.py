...
def update_user_activity(user_id: int):
    ...
    with memory_lock:
        if user_id_str in users_db:
            users_db[user_id_str]["last_active"] = now
        else:
            users_db[user_id_str] = {"first_seen": now, "last_active": now, "stt_conversion_count": 0}
def increment_processing_count(user_id: str, service_type: str):
    field_to_inc = f"{service_type}_conversion_count"
    with memory_lock:
        if user_id not in users_db:
            users_db[user_id] = {"first_seen": datetime.now(), "last_active": datetime.now()}
        users_db[user_id][field_to_inc] = users_db[user_id].get(field_to_inc, 0) + 1
...
def set_stt_user_lang(user_id: str, lang_code: str):
    with memory_lock:
        if user_id not in users_db:
            users_db[user_id] = {"first_seen": datetime.now(), "last_active": datetime.now()}
        users_db[user_id]["stt_language"] = lang_code
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    if file_size and file_size > TELEGRAM_MAX_BYTES:
        ...
        text = (
            ...
            "Please use the bot @MediaToTextBot to upload larger files (supports up to 1GB).\n"
            "Open: https://t.me/MediaToTextBot"
        )
        ...
        try:
            ...
            text = (
              "Commands supported:\n"
                ...
                "If it's larger than Telegram limits, use @MediaToTextBot (supports up to 1GB). Need more help? Contact: @boyso20"  
            )
    ...
    def handle_new_chat_members(message):
        try:
            if message.new_chat_members[0].id == bot_obj.get_me().id:
                ...
                with memory_lock:
                    groups_db[group_data['_id']] = group_data
    ...
    def handle_left_chat_member(message):
        try:
            if message.left_chat_member.id == bot_obj.get_me().id:
                with memory_lock:
                    groups_db.pop(str(message.chat.id), None)
        except Exception:
    ...
    def handle_media_types(message):
        try:
            if message.chat.id == ADMIN_ID and admin_broadcast_state.get(message.chat.id, False):
                ...
                with memory_lock:
                    all_users_chat_ids = list(users_db.keys())
                sent_count = 0
                failed_count = 0
