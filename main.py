        ...
        if content_type and content_type.startswith("application/json"):
            ...
            if raw_json is None:
                try:
                    ...
                    raw_json = None
            bot_index = request.args.get("bot_index")
            if not bot_index and isinstance(raw_json, dict):
                bot_index = raw_json.get("bot_index")
            ...
            if bot_index_val < 0 or bot_index_val >= len(bots):
                return abort(404)
            try:
                update = telebot.types.Update.de_json(raw_json)
...
def background_startup():
    ...
    try:
        ...
        logging.exception("Failed during background startup")
    try:
