...
WEBHOOK_BASE = os.environ.get("WEBHOOK_BASE", "whole-rosetta-hhhgafa-2b321fb9.koyeb.app/")
...
def delete_transcription_later(user_id: str, message_id: int):
    ...
    with memory_lock:
        ...
        if user_id in user_transcription_usage and message_id in user_transcription_usage[user_id]:
            del user_transcription_usage[user_id][message_id]
    ...
    def get_key_points_callback(call):
        try:
            ...
            if not stored:
                ...
                return
            usage = user_transcription_usage.setdefault(uid_key, {}).get(msg_id, 0) + 1
            user_transcription_usage.setdefault(uid_key, {})[msg_id] = usage
            remove_after = usage > 1
            ...
            if remove_after:
                try:
                    if uid_key in user_transcriptions and msg_id in user_transcriptions[uid_key]:
                        del user_transcriptions[uid_key][msg_id]
                    if uid_key in user_transcription_usage and msg_id in user_transcription_usage[uid_key]:
                        del user_transcription_usage[uid_key][msg_id]
                    try:
                        bot_obj.edit_message_reply_markup(chat_id_val, msg_id, reply_markup=None)
                    except Exception:
                        pass
                except Exception:
                    pass
