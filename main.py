...
transcriptions_collection = db["transcriptions"] if db else None

TRANSCRIPTION_TTL_SECONDS = 24 * 3600
if transcriptions_collection is not None:
    try:
        transcriptions_collection.create_index([("created_at", ASCENDING)], expireAfterSeconds=TRANSCRIPTION_TTL_SECONDS)
    except Exception:
        pass
...
def in_memory_action_counters():
    if "action_counters" not in in_memory_data:
        in_memory_data["action_counters"] = {}
    return in_memory_data["action_counters"]

def get_action_count(user_id: str, action: str) -> int:
    if users_collection:
        u = users_collection.find_one({"_id": user_id})
        if u and f"action_{action}_count" in u:
            try:
                return int(u.get(f"action_{action}_count", 0))
            except Exception:
                return 0
    d = in_memory_action_counters()
    with memory_lock:
        return d.get(user_id, {}).get(action, 0)

def increment_action_count(user_id: str, action: str):
    if users_collection:
        users_collection.update_one({"_id": user_id}, {"$inc": {f"action_{action}_count": 1}}, upsert=True)
        return
    d = in_memory_action_counters()
    with memory_lock:
        usermap = d.setdefault(user_id, {})
        usermap[action] = usermap.get(action, 0) + 1

def save_transcription_to_db(chat_id: int, message_id: int, text: str):
    uid = str(chat_id)
    mid = int(message_id)
    with memory_lock:
        user_transcriptions.setdefault(uid, {})[mid] = {"text": text, "created_at": datetime.utcnow()}
    if transcriptions_collection is not None:
        try:
...
def get_saved_transcription(chat_id: int, message_id: int):
    uid = str(chat_id)
    mid = int(message_id)
    with memory_lock:
        if uid in user_transcriptions and mid in user_transcriptions[uid]:
            return user_transcriptions[uid][mid]["text"]
    if transcriptions_collection is not None:
        try:
            doc = transcriptions_collection.find_one({"_id": f"{uid}:{mid}"})
            if doc:
                return doc.get("text")
        except Exception:
            pass
...
def delete_saved_transcription(chat_id: int, message_id: int):
    uid = str(chat_id)
    mid = int(message_id)
    with memory_lock:
        try:
            if uid in user_transcriptions and mid in user_transcriptions[uid]:
                del user_transcriptions[uid][mid]
        except Exception:
            pass
    if transcriptions_collection is not None:
        try:
            transcriptions_collection.delete_one({"_id": f"{uid}:{mid}"})
        except Exception:
            pass

def delete_transcription_later(user_id: str, message_id: int):
    time.sleep(TRANSCRIPTION_TTL_SECONDS)
    try:
        delete_saved_transcription(int(user_id), int(message_id))
    except Exception:
        pass
...
def build_action_markup(chat_id: int, message_id: int, corrected_text_len: int):
    uid = str(chat_id)
    markup = InlineKeyboardMarkup()
    add_get = corrected_text_len > 1000 and get_action_count(uid, "get_summary") < MAX_ACTION_USES_BEFORE_HIDE
    add_clean = corrected_text_len > 1000 and get_action_count(uid, "clean_transcript") < MAX_ACTION_USES_BEFORE_HIDE
    if add_get:
        markup.add(InlineKeyboardButton("Get Summarize", callback_data=f"get_key_points|{chat_id}|{message_id}"))
    if add_clean:
        markup.add(InlineKeyboardButton("⭐Clean transcript", callback_data=f"clean_up|{chat_id}|{message_id}"))
    return markup
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    try:
        ...
        try:
            processing_msg = bot_obj.send_message(message.chat.id, "🔄 Processing...")
        except Exception:
            processing_msg = None
        ...
        try:
            ...
            if len(corrected_text) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        if sent:
                            save_transcription_to_db(message.chat.id, sent.message_id, corrected_text)
                            threading.Thread(target=delete_transcription_later, args=(uid_key, sent.message_id), daemon=True).start()
                    except Exception:
                        pass
                    try:
                        markup = build_action_markup(message.chat.id, sent.message_id if sent else processing_msg_id, len(corrected_text))
                        if sent:
                            try:
                    ...
                    try:
                        if last_sent:
                            save_transcription_to_db(message.chat.id, last_sent.message_id, corrected_text)
                            threading.Thread(target=delete_transcription_later, args=(uid_key, last_sent.message_id), daemon=True).start()
                    except Exception:
                        pass
                    try:
                        markup = build_action_markup(message.chat.id, last_sent.message_id, len(corrected_text))
                        try:
                ...
                try:
                    sent_msg = bot_obj.send_message(message.chat.id, corrected_text or "No transcription text was returned.", reply_to_message_id=message.message_id)
                except Exception:
                    sent_msg = None
                try:
                    if sent_msg:
                        save_transcription_to_db(message.chat.id, sent_msg.message_id, corrected_text)
                        threading.Thread(target=delete_transcription_later, args=(uid_key, sent_msg.message_id), daemon=True).start()
                except Exception:
                    pass
                try:
                    markup = build_action_markup(message.chat.id, sent_msg.message_id if sent_msg else processing_msg_id, len(corrected_text))
                    if sent_msg:
                        try:
            ...
            try:
                ...
                if len(corrected_text) > 4000:
                    if user_mode == "file":
                        ...
                        try:
                            save_transcription_to_db(chat_id_inner, sent_msg.message_id, corrected_text)
                            threading.Thread(target=delete_transcription_later, args=(uid_key, sent_msg.message_id), daemon=True).start()
                        except Exception:
                            pass
                    else:
                ...
                try:
                    if sent_msg:
                        markup = build_action_markup(chat_id_inner, sent_msg.message_id, len(corrected_text))
                        try:
...
def register_handlers(bot_obj, bot_token, bot_index):
        ...
        try:
            ...
            text = (
              "Commands supported:\n"
                ...
                "If it's larger than Telegram limits, you'll be provided a secure web upload link (supports up to 250MB) Need more help? Contact admin"  
            )
    ...
    def handle_media_types(message):
        try:
            if message.chat.id == ADMIN_ID and admin_broadcast_state.get(message.chat.id, False):
                ...
                all_users_chat_ids = users_collection.distinct("_id") if users_collection else []
                sent_count = 0
                failed_count = 0
    ...
    def get_key_points_callback(call):
        try:
            ...
            stored = get_saved_transcription(chat_id_val, msg_id)
            if not stored:
            ...
            if not summary:
                ...
                try:
                    ...
                    pass
            try:
                save_transcription_to_db(call.message.chat.id, status_msg.message_id, summary)
                threading.Thread(target=delete_transcription_later, args=(str(call.message.chat.id), status_msg.message_id), daemon=True).start()
            except Exception:
                pass
            try:
                increment_action_count(uid_key, "get_summary")
            except Exception:
                pass
            try:
                orig_text = get_saved_transcription(chat_id_val, msg_id) or ""
                new_markup = build_action_markup(chat_id_val, msg_id, len(orig_text))
                try:
                    bots[0].edit_message_reply_markup(chat_id_val, msg_id, reply_markup=new_markup)
                except Exception:
                    try:
                        bot_obj.edit_message_reply_markup(chat_id_val, msg_id, reply_markup=new_markup)
                    except Exception:
                        pass
            except Exception:
                pass
        ...
        try:
            ...
            if len(cleaned) > 4000:
                if user_mode == "file":
                    ...
                    try:
                        save_transcription_to_db(call.message.chat.id, sent.message_id, cleaned)
                    ...
                    try:
                        save_transcription_to_db(call.message.chat.id, last_sent.message_id, cleaned)
                        threading.Thread(target=delete_transcription_later, args=(str(call.message.chat.id), last_sent.message_id), daemon=True).start()
                    except Exception:
                        pass
            else:
                try:
                    bot_obj.edit_message_text(f"{cleaned}", chat_id=call.message.chat.id, message_id=status_msg.message_id)
                    save_transcription_to_db(call.message.chat.id, status_msg.message_id, cleaned)
                    threading.Thread(target=delete_transcription_later, args=(str(call.message.chat.id), status_msg.message_id), daemon=True).start()
                except Exception:
                    pass
            try:
                increment_action_count(uid_key, "clean_transcript")
            except Exception:
                pass
            try:
                orig_text = get_saved_transcription(chat_id_val, msg_id) or ""
...
def ensure_webhooks_and_db():
    try:
        set_webhook_on_startup()
    except Exception:
        logging.exception("Failed to set webhooks on first request")
    try:
        if client:
            client.admin.command('ping')
            logging.info("Successfully connected to MongoDB on startup")
    except Exception:
        pass
