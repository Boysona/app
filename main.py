...
BOT_TOKENS = [
    ...
    "7790991731:AAF4NHm0BJCf08JTdBaUWKzwfs82_Y9Ecw",
]
...
button_usage_collection = db["button_usage"]
...
def animate_processing_message(bot_obj, chat_id, message_id, stop_event):
    ...
    while not stop_event():
        try:
            text = f"🔄 Processing{dots[idx % len(dots)]}"
            if text != last_text:
                bot_obj.edit_message_text(text, chat_id=chat_id, message_id=message_id)
                last_text = text
        except Exception:
            pass
        idx = (idx + 1) % len(dots)
...
def check_subscription(bot_obj, user_id: int) -> bool:
    ...
    try:
        ...
        logging.error("Error checking subscription")
        return False

def send_subscription_message(bot_obj, chat_id: int):
    if not REQUIRED_CHANNEL or not REQUIRED_CHANNEL.strip():
        return
    try:
    ...
    try:
        ...
        logging.exception("Error sending subscription message")

def consume_callback(bot_obj, call) -> bool:
    try:
        data = call.data or ""
        if not data:
            return True
        row = button_usage_collection.find_one({"_id": data})
        count = row.get("count", 0) if row else 0
        if count >= 2:
            try:
                bot_obj.answer_callback_query(call.id, "This button has reached its usage limit", show_alert=True)
            except Exception:
                pass
            return False
        button_usage_collection.update_one({"_id": data}, {"$inc": {"count": 1}}, upsert=True)
        return True
    except Exception:
        logging.exception("Error in consume_callback")
        return True
...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    if message.chat.type == 'private' and not check_subscription(bot_obj, message.from_user.id):
        send_subscription_message(bot_obj, message.chat.id)
        return
    file_id = None
    file_size = None
    filename = None
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                if not check_subscription(bot_to_use, chat_id_inner):
                    try:
                        send_subscription_message(bot_to_use, chat_id_inner)
                    except Exception:
                        pass
                    return
            except Exception:
...
def register_handlers(bot_obj, bot_token, bot_index):
    ...
    def start_handler(message):
        try:
            ...
            if chat_id == ADMIN_ID and message.text.lower() == '/admin':
                if not check_subscription(bot_obj, chat_id):
                    send_subscription_message(bot_obj, chat_id)
                    return
                bot_obj.send_message(
                    chat_id,
    ...
    def start_select_lang_callback(call):
        try:
            if not consume_callback(bot_obj, call):
                return
