...
def handle_media_common(message, bot_obj, bot_token, bot_index=0):
    ...
    if message.chat.type == 'private' and message.from_user.id != ADMIN_ID:
        try:
            member = bot_obj.get_chat_member(REQUIRED_CHANNEL, message.from_user.id)
            if member.status not in ['member', 'administrator', 'creator']:
                markup = InlineKeyboardMarkup()
                markup.add(
                    InlineKeyboardButton("Click here to join the group ", url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}")
                )
                bot_obj.send_message(message.chat.id, "🔒 Access Locked To use this bot please Join to this group:", reply_markup=markup)
                return
        except Exception:
            markup = InlineKeyboardMarkup()
            markup.add(
    ...
    def get_key_points_callback(call):
        try:
            if call.message.chat.type == 'private' and call.from_user.id != ADMIN_ID:
                try:
                    member = bot_obj.get_chat_member(REQUIRED_CHANNEL, call.from_user.id)
                    if member.status not in ['member', 'administrator', 'creator']:
                        markup = InlineKeyboardMarkup()
                        markup.add(
                            InlineKeyboardButton("Click here to join the group ", url=f"https://t.me/{REQUIRED_CHANNEL.lstrip('@')}")
                        )
                        bot_obj.send_message(call.message.chat.id, "🔒 Access Locked To use this bot please Join to this group:", reply_markup=markup)
                        bot_obj.answer_callback_query(call.id)
                        return
                except Exception:
                    markup = InlineKeyboardMarkup()
