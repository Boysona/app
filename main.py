...
for token in BOT_TOKENS:
    if LOCAL_TELEGRAM_API_URL:
        bots.append(telebot.TeleBot(token, threaded=True, parse_mode='HTML', base_url=LOCAL_TELEGRAM_API_URL))
    else:
...
def telegram_file_info_and_url(bot_token: str, file_id):
    ...
    base_api = LOCAL_TELEGRAM_API_URL or "https://api.telegram.org"
    if base_api.endswith("/"):
        base_api = base_api[:-1]
    url = f"{base_api}/bot{bot_token}/getFile?file_id={file_id}"
    resp = requests.get(url, timeout=REQUEST_TIMEOUT_TELEGRAM)
    resp.raise_for_status()
    j = resp.json()
    file_path = j.get("result", {}).get("file_path")
    if LOCAL_TELEGRAM_API_URL:
        file_url = base_api + f"/file/bot{bot_token}/{file_path}"
    else:
...
<html>
  ...
  <body>
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <br><br>
      <input type="submit" value="Upload (Max {{ max_mb }} MB)">
    </form>
  </body>
</html>
"""
...
def upload_large_file(token):
    ...
    def process_uploaded_file(chat_id_inner, lang_inner, path, bot_index_inner):
        try:
            ...
            try:
                send_summary_or_file(bot_to_use, chat_id_inner, summary)
