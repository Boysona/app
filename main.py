...
BOT_TOKENS = []
env_tokens = os.environ.get(“BOT_TOKENS”)
if env_tokens:
...
buttons.append(InlineKeyboardButton(“Get Summarize”, callback_data=f”get_key_points|{message.chat.id}|{sent.message_id if sent else processing_msg_id}”))
buttons.append(InlineKeyboardButton(“⭐Clean transcript”, callback_data=f”clean_up|{message.chat.id}|{sent.message_id if sent else processing_msg_id}”))
for b in buttons:
markup.add(b)
if sent:
try:
...
buttons.append(InlineKeyboardButton(“⭐Clean transcript”, callback_data=f”clean_up|{message.chat.id}|{last_sent.message_id}”))
for b in buttons:
markup.add(b)
try:
...
buttons = []
if len(corrected_text) > 1000:
buttons.append(InlineKeyboardButton(“Get Summarize”, callback_data=f”get_key_points|{message.chat.id}|{sent_msg.message_id if sent_msg else processing_msg_id}”))
buttons.append(InlineKeyboardButton(“⭐Clean transcript”, callback_data=f”clean_up|{message.chat.id}|{sent_msg.message_id if sent_msg else processing_msg_id}”))
for b in buttons:
markup.add(b)
if sent_msg:
try:
...
def get_key_points_callback(call):
    try:
        ...
        try:
            uid_key = str(chat_id_val)
            user_transcriptions.setdefault(uid_key, {})[status_msg.message_id] = summary
...
logging.error(“Could not connect to MongoDB on startup: %s”, e)
